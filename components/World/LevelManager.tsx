...
import { GameObject, ObjectType, LANE_WIDTH, SPAWN_DISTANCE, REMOVE_DISTANCE, GameStatus, GALAXY_COLORS } from '../../types';
...
// Trophy/Collectible Geometries
const TROPHY_BASE_GEO = new THREE.CylinderGeometry(0.2, 0.25, 0.1, 8);
const TROPHY_STEM_GEO = new THREE.CylinderGeometry(0.05, 0.05, 0.3, 8);
const TROPHY_CUP_GEO = new THREE.CylinderGeometry(0.25, 0.1, 0.4, 8);
const TROPHY_HANDLE_GEO = new THREE.TorusGeometry(0.12, 0.04, 8, 16, Math.PI);
        ...
        <group ref={groupRef} position={[data.position[0], 0, data.position[2]]}>
            ...
            <group ref={visualRef} position={[0, data.position[1], 0]}>
                ...
                {data.type === ObjectType.GEM && (
                    <group>
                        {/* Base */}
                        <mesh position={[0, -0.25, 0]} geometry={TROPHY_BASE_GEO}>
                            <meshStandardMaterial color={data.color} metalness={1} roughness={0.2} emissive={data.color} emissiveIntensity={0.5} />
                        </mesh>
                        {/* Stem */}
                        <mesh position={[0, -0.05, 0]} geometry={TROPHY_STEM_GEO}>
                            <meshStandardMaterial color={data.color} metalness={1} roughness={0.2} emissive={data.color} emissiveIntensity={0.5} />
                        </mesh>
                        {/* Cup */}
                        <mesh position={[0, 0.2, 0]} geometry={TROPHY_CUP_GEO}>
                            <meshStandardMaterial color={data.color} metalness={1} roughness={0.2} emissive={data.color} emissiveIntensity={1} />
                        </mesh>
                        {/* Handles */}
                        <mesh position={[0.2, 0.3, 0]} rotation={[0, 0, -Math.PI / 2]} geometry={TROPHY_HANDLE_GEO}>
                             <meshStandardMaterial color={data.color} metalness={1} roughness={0.2} />
                        </mesh>
                        <mesh position={[-0.2, 0.3, 0]} rotation={[0, 0, Math.PI / 2]} geometry={TROPHY_HANDLE_GEO}>
                             <meshStandardMaterial color={data.color} metalness={1} roughness={0.2} />
                        </mesh>
